...
export interface TranslationConfig {
     ...
     *          - sheetPageName.json
     */
    localePath: string; // 项目的翻译目录
    workBookId: string; // 翻译文档workId
    operatorId: string;
    appKey: string;
    appSecret: string;
    translate: {
    ...
    gptKey: string;
    gptModel?: string;
    setMaxNum?: number; // 一次性最多插入多少条数据
}

export interface TranslationLangConfig {
    text: string; // 定义表格里面 对应语言的文案
    value: string; // 对应google翻译的目标语言，例如 zh-CN、en-US
    dirText: string; // 每个语言对应项目中的目录名称，例如 google翻译对应zh-CN对应的翻译目录可能是zh或zh-CN或...
    cellKey: string; // 在表格之中每个语言对应的列号，例如：F、G
}
// 系统参数
